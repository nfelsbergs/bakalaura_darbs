...
save.image(file = "whole_image_500.rda")
...
with_col_50 <- data.frame(graph = "with_collider"
                       ...
                       , estimate = c(3.31, 4.98, 4.94, 6.10, 5.05, 4.96, 5.96, 5.93, 6.07)
                       , lower = c(-1.39, -0.92, -2.52, 1.51, 0.28, 0.34, 4.50, 4.48, 3.76)
                       , upper = c(8.01, 10.88, 12.39, 10.69, 9.83, 9.78, 7.42, 7.39, 8.38)
)

without_col_50 <- data.frame(graph = "without_collider"
                          , method = factor(c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate")
                                            , levels = c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate"))
                          , estimate = c(10.21, 10.01, 9.25, 9.97, 9.99, 10, 10.09, 10.16, 10.26)
                          , lower = c(10.08, 8.78, 8.24, 8.90, 9.10, 9.83, 9.86, 9.90, 10.01)
                          , upper = c(10.35, 11.25, 10.28, 11.05, 10.88, 10.18, 10.31, 10.42, 10.52)
)

full_50 <- data.frame(graph = "without_collider"
                          , method = factor(c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate")
                                            , levels = c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate"))
                          , estimate = c(7.68, 10, 9.25, 10, 9.99, 9.99, 10, 10, 7.71)
                          , lower = c(7.68, 9.75, 7.76, 9.62, 9.69, 9.69, 9.75, 9.75, 7.71)
                          , upper = c(7.68, 10.24, 10.73, 10.37, 10.30, 10.30, 10.24, 10.24, 7.71)
)
...
with_col_500 <- data.frame(graph = "with_collider"
                          , method = factor(c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate")
                                            , levels = c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate"))
                          , estimate = c(3.32, 4.97, 5.00, 6.13, 4.96, 4.96, 6.13, 6.13, 6.11)
                          , lower = c(1.90, 3.22, 2.77, 4.81, 3.58, 3.58, 5.68, 5.68, 4.69)
                          , upper = c(4.75, 6.72, 7.24, 7.45, 6.35, 6.35, 6.57, 6.57, 7.53)
)

without_col_500 <- data.frame(graph = "without_collider"
                             , method = factor(c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate")
                                               , levels = c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate"))
                             , estimate = c(10.22, 10.01, 9.26, 10.00, 10, 10, 10.02, 10.04, 10.40)
                             , lower = c(10.17, 9.64, 8.95, 9.68, 9.75, 9.95, 9.93, 9.84, 10.31)
                             , upper = c(10.26, 10.38, 9.56, 10.31, 10.25, 10.05, 10.12, 10.24, 10.48)
)

full_500 <- data.frame(graph = "without_collider"
                      , method = factor(c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate")
                                        , levels = c("Tiesais novertejums", "Backdoor path", "Common cause", "Pre-treatment", "Disjunctive cause", "Kopsavilkums", "Backward selection", "Forward selection", "Change-in-estimate"))
                      , estimate = c(7.68, 10, 9.23, 10, 10, 10, 10, 10, 7.71)
                      , lower = c(7.68, 9.93, 8.78, 9.91, 9.91, 9.91, 9.93, 9.93, 7.71)
                      , upper = c(7.68, 10.07, 9.67, 10.09, 10.09, 10.09, 10.07, 10.07, 7.71)
)
...
plot1 <- with_col_50 %>% ggplot(aes(x=method,y = estimate)) + geom_point(lwd = 2)+
  ...
  ylab("Novertejums tiklam ar sadursmes virsotni") +
  xlab("") +
  ggtitle("N = 50")  +
  theme(plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 5, linetype = "dashed") + coord_flip()
plot1
...
plot3 <- without_col_50 %>% ggplot(aes(x=method,y = estimate)) + geom_point(lwd = 2)+
  ...
  theme(axis.text.y = element_text(face="bold",angle = 0, vjust = 0.5, hjust=1, 
                                   size=10)) +
  # theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ylab("Novertejums tiklam bez sadursmes virsotnes") +
  xlab("") +
  ggtitle("") +
  geom_hline(yintercept = 10, linetype = "dashed") + coord_flip()
plot3
...
plot5 <- full_50 %>% ggplot(aes(x=method,y = estimate)) + geom_point(lwd = 2)+
  ...
  theme(axis.text.y = element_text(face="bold",angle = 0, vjust = 0.5, hjust=1, 
                                   size=10)) +
  ylab("Novertejums tiklam ar pilnu informaciju ") +
  xlab("") +
  ggtitle("") +
  geom_hline(yintercept = 10, linetype = "dashed") + coord_flip()
plot5
